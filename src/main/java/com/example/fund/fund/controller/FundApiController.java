...
//    }
    ...
    public ResponseEntity<ApiResponse<FundListResponse>> getFundList(
            ...
            @RequestParam(defaultValue = "false") boolean active   // ▶ NEW: 추가
    ) {
        try {
            // 1. 투자성향 유효성 검사
            if (investType < MIN_INVEST_TYPE || investType > MAX_INVEST_TYPE) {
            ...
            // 3. 필터 조건 로깅
            log.info("펀드 목록 조회 요청 - investType: {}, risk: {}, type: {}, region: {}, page: {}, active: {}",
                    investType, risk, type, region, page, active);
            ...
            // ────────────────────────────────
            // 5. ▶ NEW: active=true 이고 필터가 없으면 공개된 펀드만 조회
            if (!hasFilters && active) {
                Pageable pubPageable = PageRequest.of(page - 1, size, Sort.by("pubId").descending());

                // ✅ 서비스에서 "공개펀드 + 최신 수익률" 을 DTO로 반환
                Page<FundPolicyResponseDTO> dtoPage = fundService.findActivePublicWithReturns(pubPageable);

                FundListResponse fundListResponse = FundListResponse.builder()
                        .funds(dtoPage.getContent())
                        .investType(investType)
                        .investTypeName(getInvestTypeName(investType))
                        .build();

                PaginationInfo paginationInfo = PaginationInfo.from(dtoPage, page);
                return ResponseEntity.ok(
                        ApiResponse.success(
                                fundListResponse,
                                String.format("공개된 펀드 %d개를 조회했습니다.", dtoPage.getNumberOfElements()),
                                paginationInfo
                        )
                );
            }
